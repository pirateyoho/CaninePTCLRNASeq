...
# Purpose
To perform GSVA and single-sample GSEA comparing the gene expression profile of canine CD4 PTCL to various gene sets of human thymocyte and T-cell development available on the Molecular Signatures Database.
...
#Import GMT file
gmt <- getGmt("../GSEA/Input/Cell_of_Origin/combined_cellOrigin_gmt.gmt", geneIdType = SymbolIdentifier())
...
# GSVA Analysis
```{r GSVA, message=FALSE, fig.height=10, fig.width=12, fig.fullwidth=TRUE}
gsva_results <- gsva(
...
```{r ssGSEA, results=FALSE, fig.height=10, fig.width=12, fig.fullwidth=TRUE}
ssgsea_results <- gsva(
  vsd_matrix, # count matrix
...
# Export results
```{r file-export}
write.csv(gsva_results, "gsva_allCellOriginMSigDB_CD4PTCL.csv")
write.csv(ssgsea_results, "ssgsea_allCellOriginMSigDB_CD4PTCL.csv")
```
